...
SIDRA sums and are kept indivisible. Queries supplied only through `api =`
cannot be split because their original argument structure is unavailable.
Relative periods, period ranges, and embedded comma lists must first be
expanded to one explicit member per vector element. A geographic filter can
...
supports one geographic level, explicit or latest periods, standard
variable/category selections, and the default format and precision. It
